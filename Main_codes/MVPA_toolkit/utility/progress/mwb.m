function cancel = mwb( label, varargin )
%mwb: add, remove or update an entry on the multi waitbar
%
%   mwb(LABEL,VALUE) adds a waitbar for the specified label, or
...
%
%   mwb(LABEL,COMMAND,VALUE,...)  or
%   mwb(LABEL,VALUE,COMMAND,VALUE,...)
...
%
%   cancel = mwb(LABEL,VALUE) also returns whether the user has
...
%
%   mwb('CLOSEALL') closes the waitbar window.
%
%   Example:
%   mwb( 'CloseAll' );
%   mwb( 'Task 1', 0 );
%   mwb( 'Task 2', 0.5, 'Color', 'b' );
%   mwb( 'Task 3', 'Busy');
%   mwb( 'Task 1', 'Value', 0.1 );
%   mwb( 'Task 2', 'Increment', 0.2 );
%   mwb( 'Task 3', 'Reset' ); % Disables "busy" mode
%   mwb( 'Task 3', 'Value', 0.3 );
%   mwb( 'Task 2', 'Close' );
%   mwb( 'Task 3', 'Close' );
%   mwb( 'Task 1', 'Close' );
%
%   Example:
%   mwb( 'Task 1', 0, 'CancelFcn', @(a,b) disp( ['Cancel ',a] ) );
%   for ii=1:100
%      abort = mwb( 'Task 1', ii/100 );
%      if abort
...
%   Example:
%   mwb( 'CloseAll' );
%   mwb( 'Red...',    7/7, 'Color', [0.8 0.0 0.1] );
%   mwb( 'Orange...', 6/7, 'Color', [1.0 0.4 0.0] );
%   mwb( 'Yellow...', 5/7, 'Color', [0.9 0.8 0.2] );
%   mwb( 'Green...',  4/7, 'Color', [0.2 0.9 0.3] );
%   mwb( 'Blue...',   3/7, 'Color', [0.1 0.5 0.8] );
%   mwb( 'Indigo...', 2/7, 'Color', [0.4 0.1 0.5] );
%   mwb( 'Violet...', 1/7, 'Color', [0.8 0.4 0.9] );
%
...
% %% Example:
% 
% mwb( ['Subjects', num2str(1)], 0,'Color', 'b', 'CancelFcn', @(a,b) disp( ['Cancel ',a] ));
%   
% %  mwb( ['Subjects', num2str(1)], 0,'Color', 'b');
%  for ii=1:5
%     abort = mwb( ['Subjects', num2str(ii)], ii/5 );
%     if abort
%         break
%     else
%         pause( 1 )
%     end
%      mwb( 'Processes', 0,'Color', 'y',  'CancelFcn', @(a,b) disp( ['Cancel ',a] ));    
% %      mwb( 'Processes', 0,'Color', 'y')
%     for i=1:10
%         abort = mwb( 'Processes', i/10);
%         if abort
%              break
%         else
%             pause( 1 )
...
%     end
%    
%     mwb(['Subjects', num2str(ii)], 'Relabel', ['Subjects', num2str(ii+1)])
% end
%  
% mwb( 'CloseAll' );
...
if ~ischar( label )
    error( 'mwb:BadArg', 'LABEL must be the name of the progress entry (i.e. a string)' );
end

% Try to get hold of the figure
if isempty( FIGH ) || ~ishandle( FIGH )
    FIGH = findall( 0, 'Type', 'figure', 'Tag', 'mwb:Figure' );
    if isempty(FIGH)
        FIGH = iCreateFig();
    else
        FIGH = handle( FIGH(1) );
    ...
    for ii=1:numel( params )
        switch upper( params{ii} )
            ...
            case {'RELABEL', 'UPDATELABEL'}
                ...
                if ~ischar( values{ii} )
                    error( 'mwb:BadString', 'Value for ''Relabel'' must be a string.' );
                end
                if ismember( values{ii}, {entries.Label} )
                    error( 'mwb:NameAlreadyExists', 'Cannot relabel an entry to a label that already exists.' );
                end
            ...
            case {'CANCANCEL'}
                if ~ischar( values{ii} ) || ~any( strcmpi( values{ii}, {'on','off'} ) )
                    error( 'mwb:BadString', 'Parameter ''CanCancel'' must be a ''on'' or ''off''.' );
                end
            ...
            case {'CANCELFCN'}
                if ~isa( values{ii}, 'function_handle' )
                    error( 'mwb:BadFunction', 'Parameter ''CancelFcn'' must be a valid function handle.' );
                end
            ...
            otherwise
                error( 'mwb:BadArg', 'Unrecognized command: ''%s''', params{ii} );
...
while idx <= nargin
    param = varargin{idx};
    if ~ischar( param )
        error( 'mwb:BadSyntax', 'Additional properties must be supplied as property-value pairs' );
    end
...
if isempty( params )
    error( 'mwb:BadSyntax', 'Must specify a value or a command' );
end
end % iParseInputs
...
if ischar(baseColor)
    ...
    elseif isa( baseColor, 'double' )
        if any(baseColor>1) || any(baseColor<0)
            error( 'mwb:BadColorValue', 'Color values must be in the range 0 to 1 inclusive.' );
        end
    else
        error( badColorErrorID, badColorErrorMsg );
    end
