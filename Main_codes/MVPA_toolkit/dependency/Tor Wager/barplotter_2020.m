function h = barplotter_2020(data, varargin)
% :Usage:
% ::
%
%    h = barplotter(data)
...
if lineplot
    ...
    for k = 1:max(grouping)
        ...
        for j = grp
            plotcount = plotcount+1;
            hold on
            A(plotcount) = area(gca, [grouping(j)*0.5+plotcount-0.5-0.4 grouping(j)*0.5+plotcount-0.5+0.4], [means(j) means(j)], 'FaceColor', C{j});
            line(...
...
xlabel(gca, Xlabel, 'FontSize', xfontsize)
ylabel(gca, Ylabel, 'FontSize', yfontsize)
title(gca, Title, 'FontSize', tfontsize)
