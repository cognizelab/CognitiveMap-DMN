function out_lesion = mask_lesion_svr(xtest,ytest,model,out_train,temporary_file,param)
...
for i = 1:numel(unique(mask))
    ...
    onlyROI = apply_model_svr_curr(xtest(:,f1),ytest,out_train_temp,temporary_file);
    out_train_temp = out_train; 
    out_train_temp.feature_weight = out_train_temp.feature_weight(f2);
    removeROI = apply_model_svr_curr(xtest(:,f2),ytest,out_train_temp,temporary_file);
