...
%---------------------------------------------------------------------------------------------------------------------------------------------------%
%% default setting 
N = 10000; PO = 0; opt_parameter = []; doparallel = 0; text = 0; nparpool = 12; Haufe = 0; bricks_n = 50;
...
%% p value - type two
bootmean = mean(bootweight');
bootste = std(bootweight');
...
if Haufe > 0
    bootmean = mean(Haufeweight');
    bootste = std(Haufeweight');
    boot_Z = bootmean./bootste;
    boot_p_z = 2 * (1 - normcdf(abs(boot_Z)));
...
function [bootweight,Haufeweight] = boot_parallel(in,s,x,y,log,PO,opt_parameter,out,Haufe,bricks_n)   
    ...
    elseif Haufe == 3
        ...
        pred = bx * w + bias;
        N = size(bx,1);
        data_demean = bsxfun(@minus, bx, mean(bx,1)); % mean-center features
        pred_demean = pred - mean(pred); % mean-center predictions
        haufe_cov = data_demean' * pred_demean / (N - 1); % Cov(X, ŝ) haufe_cov (unnormalized) is not identical to haufe but they have a correlation coefficient of 1
        pred_var = pred_demean' * pred_demean / (N - 1); % or var(validation_pred)
        haufe_cov = haufe_cov / pred_var; % now it equals to haufe
        Haufeweight(:,1) = haufe_cov;
    else
        Haufeweight = [];
    end
end