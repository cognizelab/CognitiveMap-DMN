function [out,log] = mat_cv_new(x,y,c,model,cv,param)
...
%             -- param.xnew, subjects * features of interest (double) same order with x
%          <g> haufe transform
%             -- param.haufe
%                                   >>> 1 = YES 
%                                   >>> 0 = NO (default) 
...
%             -- R2, explanation rate of the predicted values to the actual values in each loop (regression momdel)
...
%             -- R2_folds, average explanation rate between true and predicted values across folds in each loop (regression momdel)
...
%% Data Filtering
log.model = model; ID = [];
if isfield(param, 'xnew') && ~isempty(param.xnew)
    PV_validation = [];  
    do_validation = true;
else
    do_validation = false;
...
if isfield(param, 'haufe') && ~isempty(param.haufe)
    do_haufe = true;Haufe = [];
else
    do_haufe = false;
end
...
% ------------------------------------------------------------------------------------------------------
    for k = 1:length(unique(idx))
        ...
        if do_validation
            xnew_test = param.xnew(ftest,:);
            xnew_test(all(xnew_test == 0, 2),:) = [];
        end
        ...
        if param.covariates > 0 && ~isempty(c) 
           ...
           if param.covariates == 1 
              ...
              [ytrain,ytest,~,c_y_test] = mat_regress_xy_new(ctrain,ctest,ytrain,ytest);
           elseif param.covariates == 2 
               if param.random == 3; ctrain = ctrain(f1,:); ctest = ctest(f1,:); end
              [xtrain,xtest,~,~] = mat_regress_xy_new(ctrain,ctest,xtrain,xtest);
           end
        end     
        % feature scaling 
        if param.scale == 1 
            if do_validation
            [~,xnew_test] = mat_scale(xtrain,xnew_test); 
            end
        ...
        % ------------------------------ validation test on xnew --------------------------------
        if do_validation
            output_7T = predict(temporary_file, xnew_test);
            PV_validation_rep = [PV_validation_rep; output_7T];
        end
        % -------------------------------data concatenation ----------------------
        % data concatenation
        tv = out_apply{k,n}.tv;
        ...
        PV = [PV; pv];
        PW = [PW; dp];
        %TV = [TV; out_apply{k,n}.tv + c_y_test(:)]; PV = [PV;(out_apply{k,n}.pv)+c_y_test(:)]; PW = [PW;out_apply{k,n}.dp]; 
        %---------------------------------- Haufe transform ------------------------
        if do_haufe
            N = size (pv,1);
            pv_demean = pv - mean(pv);
            pv_var = pv_demean' * pv_demean / (N - 1);
            xtest_demean = bsxfun(@minus, xtest, mean(xtest,1));
            haufe_cov = xtest_demean' * pv_demean / (N - 1); % Cov(X, ŝ) haufe_cov (unnormalized) is not identical to haufe but they have a correlation coefficient of 1
            haufe_cov = haufe_cov / pv_var; % now it equals to haufe
            Haufe = [Haufe,haufe_cov]; 
        end
    ...
    if do_validation
        PV_validation = [PV_validation,PV_validation_rep]; 
    end
% ------------------------------------------------------------------------------------------------------
%% Model Assessment ★★★
% ------------------------------------------------------------------------------------------------------   
...
if do_validation
    %out.validation_result_full = out_assess_full_new;
    %out.validation_result = out_assess_new;
    out.PV_validation = PV_validation;
    %out.PV_recovered = PV;
    %out.TV_recovered = TV;
end
